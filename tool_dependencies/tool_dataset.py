...
class PackedGraphSimilarityPairs(GraphSimilarityDataset):
    ...
    def __init__(self, db_path, graph_path, normalization_mean, normalization_std, included_files = []):
      """Constructor.
    ...
    def pairs(self, batch_size):
        ...
        for db_file in self._db_files:
          ...
          packer = db_file.split("_")[0]

          batch_graphs.append((g1, g2))
          batch_packers.append(packer)
          batch_files.append(db_file)

          if len(batch_graphs) == batch_size:
            yield self._pack_batch(batch_graphs), batch_packers, batch_files
            batch_graphs = []
            batch_packers = []
            batch_files = []
