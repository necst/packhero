...
EXPERIMENT_PATH = f"experiments_ph/trainsetsize/100/"
...
csv_file_writer.writerow(['approach', 'packer', 'samples_num', 'unknown_rate', 'false_positive_rate', 'precision', 'recall', 'f1_score', 'accuracy', 'avg_num_instances'])
...
for file in files:
    ...
    eval_metrics = {
        ...
        'unknown_rate': [round(rate, 2) for rate in unknown_rate],
        'false_positive_rate': [round((falsep[i] / (falsep[i] + truen[i])), 4) if (falsep[i] + truen[i]) > 0 else 0 for i in range(len(list(set(y_test))))],
        'precision': [round((truep[i] / (truep[i] + falsep[i])), 2) if (truep[i] + falsep[i]) > 0 else 0 for i in range(len(list(set(y_test))))],
        'recall': [round((truep[i] / (truep[i] + falsen[i])), 2) if (truep[i] + falsen[i]) > 0 else 0 for i in range(len(list(set(y_test))))],
        'f1_score': [round((2 * ((truep[i] / (truep[i] + falsep[i])) * (truep[i] / (truep[i] + falsen[i]))) / ((truep[i] / (truep[i] + falsep[i])) + (truep[i] / (truep[i] + falsen[i])))), 2) if ((truep[i] + falsen[i]) > 0 and (truep[i] + falsep[i])) else 0 for i in range(len(list(set(y_test))))],
        'accuracy': [round(((truep[i] + truen[i]) / (truep[i] + truen[i] + falsep[i] + falsen[i])), 2) for i in range(len(list(set(y_test))))],
    ...
    for index, row in metr.iterrows():
        csv_file_writer.writerow([approach, index, row['samples_num'], row['unknown_rate'], row['false_positive_rate'], row['precision'], row['recall'], row['f1_score'], row['accuracy'], row['avg_num_instances']])
